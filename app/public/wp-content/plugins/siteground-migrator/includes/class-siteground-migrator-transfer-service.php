		...
		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			...
			if (
				...
				return self::update_status(
					...
					0,
					esc_html__( 'You are using a version of WooCommerce that has a known issue preventing our plugin from migrating your content successfully. Please, update WooCommerce to version 4.2.1 or newer and start the migration anew!', 'siteground-migrator' )
				);
			}
		}
	...
	private function before_start_transfer() {
		...
		if ( 0 !== $response['status_code'] ) {
			self::update_status(
				$response['message'],
				0,
				esc_html__( 'Please check your current hosting permissions and error logs and restart the transfer.', 'siteground-migrator' )
			);
